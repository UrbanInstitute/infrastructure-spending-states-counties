<script context="module">
   ...
   * @property {string=} label - An optional label to preface the given text with
   * @property {string} text - The text to display for a line
   */
</script>

<script>
  /**
   * A list of content lines to display in the tooltip
   * @type {TooltipContent[]}
   */
  export let content;
  ...
  $: tooltipDirection = getTooltipDirection(xPosition, yPosition, windowWidth, windowHeight);
...
<div
  ...
  style={`left: ${xPosition}px; top: ${yPosition}px; width: ${sizes[size]}px;`}
>
  <!-- <svg -->
  <!--   class="tooltip-shape" -->
  ...
  <!--   /> -->
  <!-- </svg> -->
  {#each content as { label, text }}
    <p class="tooltip-text">
      {#if label}
        <span class="tooltip-label">{label}</span>
      {/if}
      <span class="tooltip-text">{@html text}</span>
    </p>
  {/each}
</div>

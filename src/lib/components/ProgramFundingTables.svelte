<script>
  ...
  function formatProgramDataForTable(program_data, funding_data) {
    const county_programs = Object.keys(funding_data).map((program) => program.replace("_per_1k", ""))
    return program_data
      .filter((program) => county_programs.includes(program.short_name) && funding_data[`${program.short_name}_per_1k`] > 0)
      .map((program) => {
      return {
  ...
  $: iija_program_funding = formatProgramDataForTable(
    $program_list.filter((program) => program.type == "IIJA"),
    data
  );
...
<div class="section-label"><p>IIJA funding</p></div>
<div class="spacing"></div>
<LayoutBlock>
  <!-- <h2 class="module-subhead">IIJA Program funding</h2> -->
  {#if iija_program_funding.length > 0}
    <Table sort_column="total_funding" data={iija_program_funding} limit_height {format_funs} />
  {:else}
